...
class CommandReadyListener implements EventSubscriberInterface
{
    ...
    public function onCommandReady(StateCommandEvent $event): void
    {
    ...
    private function sendMail(User $user): void
    {
        ...
        $message = (new Swift_Message('[ojbento.fr] : Commande Prête'))
            ->setFrom('no-reply@ojbento.fr')
            ->setTo($sendTo)
            ->setBody(
                $this->templating->render(
                    'emails/ready_command.email.twig',
                    ['user' => $user]
                ),
                'text/html'
            );
